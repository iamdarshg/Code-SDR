...
The four-layer board reserves In1.Cu and In2.Cu as solid GND references.
Signal routing therefore uses only F.Cu and B.Cu, with RF-critical nets
restricted to F.Cu unless an explicit reviewed crossing is supplied.
"""
...
# Stackup (from Code-SDR-V2.kicad_pcb): F.Cu -> 0.13mm FR4 (er=4.2) -> In1.Cu (GND).
...
#     LNA in/out, filters): W=0.23mm with 35um finished outer copper
...
# Routing layers in preference order.  Both internal layers are uninterrupted
# GND references and must never be used for signals.
...
def build_grid_and_search(obstacles, src, dst, allow_vias, net_name, extra_pad_mm=4.0,
    ...
    # The copper-edge DRC is not represented by pad/track obstacles. Clamp
    # every search box inside the actual outline so a wide detour cannot
    # leave the board and return on the far side.
    edge_box = obstacles.board.GetBoardEdgesBoundingBox()
    edge_margin = 0.25
    lo_x = max(lo_x, mm(edge_box.GetLeft()) + edge_margin)
    hi_x = min(hi_x, mm(edge_box.GetRight()) - edge_margin)
    lo_y = max(lo_y, mm(edge_box.GetTop()) + edge_margin)
    hi_y = min(hi_y, mm(edge_box.GetBottom()) - edge_margin)
    ...
    all_via_points = [
        (mm(item.GetPosition().x), mm(item.GetPosition().y))
        for item in obstacles.board.GetTracks()
    ...
    def through_via_site_clear(i, j):
        """A through-via must clear copper on every signal-routing layer.

        Checking only the source and destination layers can place the drill
        through a trace on an intermediate layer. The one-cell ring also
        accounts conservatively for the via being wider than the route trace.
        """
        x, y = wx(i), wy(j)
        if any(math.hypot(x - vx, y - vy) < 0.35 for vx, vy in all_via_points):
            return False
        for layer in layers:
            arr = blocked[layer]
            for di in (-1, 0, 1):
    ...
    while pq:
        ...
        if allow_vias:
            if not through_via_site_clear(i, j):
                continue
